...
# --- Configuration ---
my $alert_on_missing = 1;           # Set to 1 to email on missing files, 0 to only email on integrity mismatch

die "Error: OHB_VERIFY_SERVERS environment variable is not set. Please provide a comma-separated list of server URLs.\n"
    unless defined $ENV{'OHB_VERIFY_SERVERS'} && $ENV{'OHB_VERIFY_SERVERS'} ne '';

my @ohb_servers = split(',', $ENV{'OHB_VERIFY_SERVERS'});
...
sub logger {
    ...
    return if $ENV{'OHB_VERIFY_EMAILS'} && ! -t STDOUT && !$force;
...
sub send_alert {
    my ($subject, $body) = @_;
    my $to_list = $ENV{'OHB_VERIFY_EMAILS'};
...
foreach my $ohb_base_url (@ohb_servers) {
    ...
    $ohb_base_url = "http://$ohb_base_url" unless $ohb_base_url =~ /^http/;
...
Usage:
  # Default (checks ohb.hamclock.app)
  perl verify_firmware_integrity.pl

  # Check a specific instance
  OHB_VERIFY_URL="http://your-server-ip" perl verify_firmware_integrity.pl
