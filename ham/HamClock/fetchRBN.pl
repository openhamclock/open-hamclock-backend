...
sub call_to_grid {
    ...
    # RBN skimmer callsigns often have a "-#" suffix, e.g. WC2L-#.
    $call =~ s/-#$//;

    # Also strip any other "-suffix".
    # Portable indicators like /P are usually before any dash.
    # Skimmer self-spots may use -# or -1, etc.
    $call =~ s/-\w+$//;
...
csv_error(400, "maxage must be integer seconds")
    if $maxage !~ /^\d+$/;
...
if ($sel_name eq 'ofcall') {
    ...
    csv_error(400, "Unsupported selector: $sel_name");
}

if ($sel_name =~ /grid$/) {
    csv_error(400, "Invalid Maidenhead grid format")
        if $sel_value !~ /^[A-Ra-r]{2}[0-9]{2}([A-Xa-x]{2})?$/;
} else {
    csv_error(400, "Invalid callsign format")
        if $sel_value !~ /^[A-Za-z0-9\/\-]+$/;
}
...
csv_error(
    502,
    "Could not connect to dxspider feed at $DXSPIDER_HOST:$DXSPIDER_PORT: $!"
) unless $sock;
...
csv_error(502, "Empty response from dxspider feed")
    unless length $raw;
...
eval {
    ...
    1;
} or csv_error(502, "Could not parse JSON from dxspider: $@");

csv_error(
    502,
    "dxspider feed returned status: " . ($data->{status} // 'unknown')
